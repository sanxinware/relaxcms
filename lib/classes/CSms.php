...
class CSms
{
	...
	static function &GetInstance($name, $options)
	{
		...
		{	
			...
			$classfile = RPATH_SMS.DS.$name.DS.$name.".php";
...
*/
	public function send($params = array())
	{
		...
		if (!$params['appCode']) {
			rlog(RC_LOG_ERROR, __FILE__, __LINE__, __FUNCTION__, "no appCode");
			return false;
		}
		
		if (!$params['signId']){
			rlog(RC_LOG_ERROR, __FILE__, __LINE__, __FUNCTION__, "no signId");
			return false;
		}
		
		if (!$params['templateId']) {
			rlog(RC_LOG_ERROR, __FILE__, __LINE__, __FUNCTION__, "no templateId");
			return false;
		}
		
		if (!$params['phone']) {
			rlog(RC_LOG_ERROR, __FILE__, __LINE__, __FUNCTION__, "no phone");
			return false;
		}

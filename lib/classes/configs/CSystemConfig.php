...
class CSystemConfig extends CConfig
{
	...
	public function load($reload=false)
	{
		...
		//system
		empty($cfg['title']) &&  $cfg['title'] = 'RC';		
		empty($cfg['description']) &&  $cfg['description'] = 'RC';		
		empty($cfg['copyright']) &&  $cfg['copyright'] = 'RC';		
		...
		//会员超时
		!isset($cfg['session_timeout']) && $cfg['session_timeout'] = 600;
		//验证码仅数字
		...
		empty($cfg['updatetype']) && $cfg['updatetype'] = 0;
		empty($cfg['updateapi']) && $cfg['updateapi'] = 'http://localhost/api';
		
		//cross-site
		empty($cfg['xss_access']) && $cfg['xss_access'] = 0;
		//rewrite_enable
		!isset($cfg['rewrite_enable']) && $cfg['rewrite_enable'] = 1;
		
		//加密配置
		!isset($cfg['fencode']) && $cfg['fencode'] = 1;
		!isset($cfg['show_optmenu_button_title']) && $cfg['show_optmenu_button_title'] = 0;
		
		//显示序号
		!isset($cfg['show_rankid']) && $cfg['show_rankid'] = 0;
		!isset($cfg['show_id']) && $cfg['show_id'] = 1;

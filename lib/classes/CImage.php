...
class CImage
{
	...
	public function load($imageRef) 
	{
		...
		rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__, 'could not load anything. Giving up!');
		return false;
	}
	...
	public function resize($maxSize) {
		if (!$this->valid()) {
			rlog(RC_LOG_ERROR, __FILE__, __FUNCTION__, __LINE__, 'No image loaded');
			return false;
		}
	...
	public function addtag($imgfile, $text, $width=128, $height=128)
	{
		
		$szs = @getimagesize($imgfile);
		if (!$szs) {
			rlog(RC_LOG_ERROR, __FILE__, __LINE__, __FUNCTION__, "call getimagesize filed! szs=$szs, src='$imgfile'");
			return false;
		}
		...
		// 指定字体文件
		$fontPath = RPATH_SUPPORTS.DS."fonts".DS."song.ttf"; 
		if (!file_exists($font)) {
			$fontPath = RPATH_SUPPORTS.DS."fonts".DS."CourierNew.ttf"; 
		}
		rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__, $fontPath, $orig_height, $orig_width);
		// 水印文字
		//$text = '水印文字';
		
		// 分配颜色和透明度
		$color = imagecolorallocatealpha($im, 192, 192, 192, 50); // 白色透明度50
		...
		$per_height = $orig_height/10;
		$per_width = $orig_width/10;
		
		$per_height < 320 && $per_height = 320;
		$per_width < 240 && $per_width = 240;
		...
		for ($i=0; $i<10; $i++) {
			...
			for ($j=0; $j<10; $j++) {
				...
				rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__, $y, $x, $text);
				// 水印文字
				$res = imagettftext($im, $fontSize, $angle, $x, $y, $color, $fontPath, $text);
				if (!$res) {
					rlog(RC_LOG_ERROR, __FILE__, __LINE__, __FUNCTION__, "call imagettftext failed!");
				}
			}
		...
		switch ($bigType) {
			case 1:
				$res = imagegif($im, $dstfile);
				break;
			case 2:
				$res = imagejpeg($im, $dstfile);
				break;
			case 3:
				$res = imagepng($im, $dstfile);
				break;			
			default:
				break;
		}
		if (!$res) {
			rlog(RC_LOG_ERROR, __FILE__, __LINE__, __FUNCTION__, "call imagexxx write failed!bigType=$bigType.");
		}

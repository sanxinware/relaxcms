...
class CUpgradeComponent extends CFileDTComponent
{
	...
	{
		$res = $this->doUpload($options);
		showStatus($res);
		return $res;
	}

	protected function cleanUpload($uploadinfo, &$options=array())
	{
		...
		$cci18bdir = RPATH_CACHE.DS.$options['_appname'].DS.'i18n';
		if (is_dir($cci18bdir))
			s_rmdir($cci18bdir);
	...
	protected function del(&$options=array())
	{
		...
		$this->cleanUpload($uploadinfo, $options);		
		showStatus(0);		
	}
	...
	protected function doUpgradeByTgz($uploadinfo, &$options=array())
	{
		$upgrade_res = false;
		...
		//首次变更产品版本
		$is_new_product_version = (is_file($dir.DS.'web'.DS.'version.php') && !is_file(RPATH_ROOT.DS.'version.php'));
				
		//检查产品版本
		if (isset($udb['version']) && !$is_new_product_version) {
			...
			if ($verdb[0] > $verdb2[0]) {
				...
				rlog(RC_LOG_ERROR, __FILE__, __LINE__, "upgrade minor version error!current_product_version=$current_product_version,upgrade to version=".$udb['version']);	
				return false;
	...
	protected function doUpgradeLZ($uploadinfo, &$options=array())
	{
		return false;
	}

	protected function doUpgrade($uploadinfo, &$options=array())
	{
	...
	//升级
	protected function upgrade(&$options=array())
	{
		$uploadinfo = cache_array('upgrade');
		if (!$uploadinfo) {
			showStatus(-1);
		...
		$res = $this->doUpgrade($uploadinfo, $options);
		if (!$res) {
			showStatus(-1);
			return  false;			
		}

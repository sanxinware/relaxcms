...
class CDatabase 
{
	...
	*/
	 ...
	 *
	 */
	public function import($sqlfile, &$nr_success=0)
	{
	...
	public function compute($tablename, $fn, $field='', $params=array())
	{
		...
		$pkey = $this->_pkey?"$this->_pkey,":'';
		$sql = "select $pkey $fns from $tablename $where";		
		
		//rlog(RC_LOG_DEBUG, __FILE__, __LINE__, 'sql='.$sql);
		
		$res = $this->get_one($sql);
		if (!$res) {
			rlog(RC_LOG_ERROR, __FILE__, __LINE__, __FUNCTION__, "compute failed! sql=$sql");
			return false;
		}
	...
	public function insert($tablename, $params) 
	{
		...
		//rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__, $sql);
	...
	public function update($tablename, $pkey, $id, $params) 
	{
		...
		//rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__, "res=$res, sql=$sql", $params);
		
		return $res;
	}
	
	public function updateAll($tablename,$params, $wheres) 
	{
		$k2v = array();
		foreach ($params as $k=>$v) {
			$k2v[] = "`$k`='$v'";
		...
		//filter		
		$where = $this->buildFilterSQL($wheres);		
		if ($where)
			$sql .= $where;
		...
		rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__, "res=$res, sql=$sql", $wheres);
	...
	*/
	...
	protected function buildFilterSQL(&$params=array())
	{
		...
		$filter = isset($params['____filter'])?$params['____filter']:array();
		if ($filter) {
			$and_wheres = array();		
			foreach ($filter['and_wheres'] as $key=>$v) {
				$type = $v['type'];
				$op = $v['op'];
				$key = $v['as']?$v['key']:'`'.$v['key'].'`';
				$value = $v['value'];
		...
		if (isset($params['____where']) && $params['____where']) {
			$____where = trim($params['____where']);
			$where = is_start_with($____where, 'where')?$____where:" where $____where";
		}
		
		//rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__, "WHARE...", $where);
	...
	protected function buildGroupSQL(&$params=array())
	{
		...
		$groupby = '';
		if (isset($params['__groupby'])) {
			$groupby = 'group by ';
			
			if (is_array($params['__groupby'])) {
				$__fields = $params['__groupby'];
				$groupby .= ' '.implode(',', $params['__groupby']);
			} else {
				$__fields = array($params['__groupby']);
				$groupby .= $params['__groupby'];
			}
		
			$__fields[] = 'count(*) as count';
			
			$params['__fields'] = $__fields;
		}
	...
	protected function buildTableName($tablename, &$params)
	{
		//多模型联合查询
		$_tablename = $tablename;
		if (isset($params['__joinon'])) {
			/* eg: [__joinon]=>array('file2org'=>'fid') : a.* from tablename a inner join file2org b on a.id=b.fid*/
			$type = $params['__joinon']['type'];
			$b = $params['__joinon']['b'];
			$on =  $params['__joinon']['on'];
			$_tablename .= " a $type join $b b on a.id=b.$on ";
			
			//__fields
			if (!isset($params['__fields'])) {
				$fdb = array();
				foreach ($params['__joinon']['fdb'] as $key=>$v) {
					if (isset($v['as'])) {
						$v['as'] == $key && $key = 'a.'.$key;						
						$fdb[] = $key.' as '.$v['as'];
					} else {
	...
	public function select($tablename, &$params=array())
	{
		...
		$tablename = $this->buildTableName($tablename, $params);		
		//total
		$sql_total = "select count(*) as total from $tablename $where $groupby";
		$res = $this->get_one($sql_total);
		$total = intval($res['total']);
		$page_size = isset($params['page_size'])?intval($params['page_size']):0;
	...
	{
		$dbversion = $this->get_version();
		return $this->_options['dbtype'].'/'.$dbversion.'/'.$this->_options['dbcharset'].'/'.$this->_options['dbhost'].(!empty($this->_options['dbport'])?':'.$this->_options['dbport']:'');
	}

...
class CMainApplication extends CApplication
{
	...
	public function dispatch(&$options=array())
	{
		...
		if ($isLogin) {
			//$this->checkStartComponent($options);
		}
	...
	{
		//rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__, "IN ...component=$component, task=$task, appname=".$this->_name);
		$ss = $this->getSession();		
		$menus = $this->getMenus();
		if (!isset($menus[$component])) {
			rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__, "WARNING: no menuitem of component=$component");
			return true;
		}
		...
		//rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__, $component, $menuitem);
		...
		//rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__, "############### pid=$pid, component=$component, task=$task, perm=$perm ###############");
	...
	{
		$i18n = $this->getI18n();	
		$systemmenuparams = array();
		if (is_model('menu')) {
			$m1 = Factory::GetModel('menu');
			$systemmenuparams = $m1->getParams();
		}
		...
		$fname = $m['name'].'.php';
		if (!$appname) {
			if (file_exists($this->_appdir.DS.'components'.DS.$fname)) {
				$path = $this->_appdir.DS.'components'.DS.$fname;
			} else {
				if (file_exists(RPATH_COMPONENTS.DS.$fname)) {
					$path = RPATH_COMPONENTS.DS.$fname;
				}
			}
		} else {
			if (file_exists(RPATH_APPS.DS.$appname.DS.'components'.DS.$fname)) {
				$path = RPATH_APPS.DS.$appname.DS.'components'.DS.$fname;
			}		
		}
		$m['path'] = $path;
	...
	public function cacheMenus($lang='')
	{
		...
		foreach ($mdb as $key => $v) {
			...
			if (($level & $this->_default_level_mask) === 0) {
				continue;
			}
		...
		$cache = Factory::GetCache();
		$cache->cache_array('menus', $_mdb, $file);

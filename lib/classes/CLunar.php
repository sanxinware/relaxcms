...
class CLunar
{
  ...
  */
  ...
  public function getNowForView()
  {
    $data = array();

    $ts = time();
    $data['time'] = tformat($ts,'H:i');
	$data['date'] = tformat($ts, 'Y年n月j日 ').tformat_N($ts);
    ...
    $today = $this->convertSolarToLunar($y, $m, $d);
    $month = $this->getJieQi($y, $m, $d);//获取当前节气

    $data['lunar'] = $today[3]."(".$today[6].") ".$today[1]." ".$today[2]." ".$month['name2'];
...
echo "今天是农历".$today[0]."年".$today[1]."月".$today[2]."农历".$today[3]."年".$today[4]."月".$today[5]."日".$today[6]."年<br/>";

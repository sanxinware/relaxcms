...
class CSplashClientModel extends CNoTableModel
{
	...
	*/
	...
	protected function saveSplashCache($sdb)
	{
		$sdb['ts'] = time();
		//id
		array_sort_by_field($sdb, 'url');
		return cache_array('splashdb', $sdb);
	}
	
	
	protected function getListMD5String($bdb)
	{
		$urlall = 's.x';
	...
	{
		//rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__, "IN ... ", $bdb);
		
		$newmd5str = $this->getListMD5String($bdb);
				
		$oldsplashdb = $this->loadSplashCacheInfo();
		$oldts = isset($oldsplashdb['ts'])?$oldsplashdb['ts']:0;
		
		if ($newmd5str == $oldsplashdb['md5']) {
			if (!$force && time() - $oldts < RC_TIMESEC_DAY) { 
		...
		//rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__, $bdb, $fdb);
		...
		foreach ($bdb as $key=>$v) {
			...
			$data = curlGET($url);
			if (!$data) {
				rlog(RC_LOG_ERROR, __FILE__, __LINE__, __FUNCTION__, "call curlGET failed!url=$url");
				continue;
			}
			
			$res = s_write($dst, $data);
			if (!$res) {
				rlog(RC_LOG_ERROR, __FILE__, __LINE__, __FUNCTION__, "call s_write failed!dst=$dst");
			}
		}
	...
	{
		!$bgdir && $bgdir = RPATH_PUBDATA.DS.'bg';

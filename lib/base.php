...
class RC
{
	...
	public static function init() 
	{
		...
		@ini_set('memory_limit', -1); //不限 512M
	...
	protected static function render($appname, $options=array())
	{
		if (!self::isInstalled()) {
			$app = Factory::GetApplication('installation');
			if (!$app) {
				throw new CException(404, "No app 'installation'!", __FILE__, __LINE__);
			}	
			return $app->run($options);
		}		
		
		$mapp = Factory::GetApplication($appname, $options);	
		if (!$mapp) {
			//throw new CException(404, "No app '$appname'!", __FILE__, __LINE__);
			rlog(RC_LOG_DEBUG, __FILE__, __LINE__, "no app '$appname'!url=".$options['url']);
			header("HTTP/1.1 404 NOT Found!");
			exit("NOT Found APP '$appname' error!");
		}	
		return $mapp->run($options);
	}


	public static function run($appname='admin', $options=array()) 
	{
		$request = Factory::GetRequest();
		$appname = $request->dispatch($appname, $options);
		
		RC::render($appname, $options);	

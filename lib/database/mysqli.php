 ...
 *
 */
class MysqliDatabase extends CDatabase
{
	...
	function MysqliDatabase($name,  $options )
	{
	...
	public function connect()
	{
		...
		if (!$link) {
			rlog(RC_LOG_ERROR, __FILE__, __LINE__, "Connect to MySQLi failed!");
			return false;
		}
		...
		{
			mysqli_query($this->_link, "SET character_set_connection=".$this->_options['dbcharset'].", character_set_results=".$this->_options['dbcharset'].", character_set_client=binary");
		}
		...
		if ($this->_options['dbname']) 
		{
			...
			{
				rlog(RC_LOG_ERROR, __FILE__, __LINE__, "use db '{$this->_options['dbname']}' failed!", mysqli_error($this->_link));
				return false;
			}
		}		
		return true;
	...
	public function db_select($dbname=null)
	{
		...
		$res = @mysqli_select_db($this->_link, $dbname);
		if (!$res) {
			rlog(RC_LOG_ERROR, __FILE__, __LINE__, "use db '{$this->_options['dbname']}' failed!", mysqli_error($this->_link));
		}
	...
	public function db_drop($dbname)
	{
		...
		if (function_exists('mysqli_unbuffered_query'))
		{
			$query = mysqli_unbuffered_query($this->_link, $sql);
		}
		else
		{
		...
		@mysqli_free_result($this->_link, $query);
		return true;
	}
	...
	{
		$o_size = 0;
		$query = mysqli_query($this->_link, "SHOW TABLE STATUS");
	...
	public function db_exists($dbname)
	{
		...
		$h = $this->query($sql);
		if (mysqli_num_rows($this->_link, $h) ==1 )
		{
			return true;
		}
		return false;	
	...
	public function query($sql, $method = '') 
	{
		...
		if ($method =='U_B' && function_exists('mysqli_unbuffered_query'))
		{
			$query = mysqli_unbuffered_query($this->_link, $sql);
		}
		else
	...
	function halt($msg='') 
	{
		...
		rlog(RC_LOG_DEBUG, __FILE__, __LINE__, "MYSQLi Halt: errno=$errno, error=$error!");
	...
	*/
		...
		foreach($udb as $key=>$v) {
			...
			foreach($v as $k2=>$v2){
				...
				if ($v2 == "") {
					...
					$tmp .= "'".mysqli_real_escape_string($v2)."',";
			}
	...
	public function backup_in($file, $start, $count, &$total)
	{
		...
		foreach($lines as $key => $value)
		{
			...
			if(substr($value, -1) == ';') {
				...
				rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__,"call query:$sql");
			} else {
				$sql .= $value;
			}			
		}
	...
	function get_guid()
	{
		...
		$rs =& mysqli_fetch_array($this->_link, $query, MYSQL_ASSOC);		
		mysqli_free_result($this->_link, $query);		
		return $rs['guid'];
	}
	...
	public function queryFields($table_name)
	{
		...
		$fdb = array();
		if ($res) {
			$strtypes = array('char' => true ,'varchar'=>true, 'text'=>true, 'mediumtext'=>true, 'longtext'=>true, 'tinytext'=>true );
...
*/
	...
	{
		$udb = array();
		$res = mysqli_query($this->_link, "show table status where Engine !='VIEW'");
		while ($v = $this->fetch_array($res)) 
		{
			$udb[] = $v;
		}
	...
	public function escape_string($data)
	{
		...
		*/
		...
		return mysqli_real_escape_string($this->_link, $data); //mysqli_escape_string($data);
	}

 ...
 * 
 */
class SqlitepdoDatabase extends CDatabase
{
	...
	function SqlitepdoDatabase($name, $options)
	{
	...
	public function connect()
	{
		...
		*/
		try {
			$dbfile = str_replace(DS, '/', $this->_dbrootdir.DS.$this->_options['dbname']);
			//连接
			$params = array(
			...
			$link = new PDO($dsn, null, null, $params);
			if (!$link) {
				rlog(RC_LOG_ERROR, __FILE__, __LINE__, __FUNCTION__, "call PDO failed!dsn=".$dsn);
			}
		} catch (PDOException $e) {
			rlog(RC_LOG_ERROR, __FILE__, __LINE__, "new PDO failed!".$e->getMessage().', dsn='.$dsn);
			$link = false;
		}
	...
	public function query($sql, $method = '') 
	{
		...
		try {
			...
			rlog(RC_LOG_ERROR, __FILE__, __LINE__, "new PDO failed!".$e->getMessage().', sql='.$sql);
			//$this->close();
		}
	...
	function fetch_array_old($query, $result_type = SQLITE3_ASSOC) 
	{
	...
	{
		rlog(RC_LOG_ERROR, __FILE__, __LINE__, __FUNCTION__, "SQLite halt!error=".$this->_link->errorInfo());		
		return false;
	}	
	...
	*/
	...
	{
		$errorCode =  $this->_link->errorCode();

...
class MysqlpdoDatabase extends CDatabase
{
	...
	protected function halt($msg='') 
	{
		...
		rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__, "MYSQL Halt: $last_error!msg=$msg");
	...
	*/
	...
	public function exec($sql) 
	{
		...
		//rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__, "IN exec", $sql);
		
		try {
			$res = $this->_link->exec($sql);//返回受影响的行数						
		} catch (PDOException $e) {
			rlog(RC_LOG_ERROR, __FILE__, __LINE__, __FUNCTION__, "call exec failed!sql=$sql", $e->getMessage());
			$res = false;			
		}
	...
	*/
	...
	{
		$version = $this->_link->query('select version()')->fetchColumn();

...
class InstallComponent extends CUIComponent
{
	...
	{
		//updateapi
		if (file_exists(RPATH_ROOT.DS."RELAXCMS.txt") && empty($params['updateapi'])) {
			$params['updateapi'] = 'https://apm.relaxcms.com/api';
		}
		//check updateapi
		$apiurlfile = RPATH_CACHE.DS.'.apiurl';
		if (file_exists($apiurlfile)) {
			$res = s_read($apiurlfile);
			if ($res) {
				$res = trim($res);
				if ($res) {
					$params['updateapi'] = $res;
					$params['updatetype'] = 1;
				}
			}
		}
	...
	protected function init(&$options=array())
	{
		...
		$pfname = get_product_fullname();
		if (!$pfname) {
			$title = "RelaxCMS Web Installation";
			$subtitle = 'A base PHP web installation';
		} else {
			$title = $pfname." 安装";
			$subtitle = '';
		}
		...
		$this->assign('sys_install_title', $title);
		$this->assign('sys_install_subtitle', $subtitle);
	...
	public function show(&$options=array())
	{
		...
		!isset($params['dbuser']) && $params['dbuser'] = 'root';
	...
	protected function __check(&$res=array()) 
	{
		...
		$w_check = array('cache', 'data', 'config');
	...
	//检查
	protected function check(&$options=array(), $params = array())
	{
		...
		$pass = $this->__check();
		$options['nexttask'] = 'check';
		if ($params['current'] == 'check') {
			if ($this->_sbt) {
				if ($pass) { //过
					return $this->config($options, $params);				
				}
			}
		}
	}	

	protected function checkenv(&$options=array())
	{
	...
	protected function config(&$options = array(), $params = array())
	{
	...
	private function installApps($apps, $options=array())
	{
		...
		//rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__, $apps);
		
		$mainapp = Factory::GetApp('admin');
		$mainapp->install($options);
	...
	protected function install(&$options=array())
	{
		...
		$syscfg['webroot'] = $options["_webroot"];
		...
		$res = $this->installApps($params['app'], $options);
		
		$options['nexttask'] =  'install';
			
		return $res;
	}
	
	protected function finished(&$options = array() )
	{
	...
	protected function setHtaccess($options)
	{
		return set_htaccess($options['_webroot']);		
	}

	protected function setup(&$options = array())
	{
		$res = $this->install($options);
		if ($res) {
			//创建 .htaccess
			$this->setHtaccess($options);			
			touch(RPATH_CONFIG.DS.'installed');
		}		
		showStatus($res);
	}

...
class ApiComponent extends CUIComponent
{
	...
	protected function redirectForAPI(&$options=array())
	{
		$cf = get_config();
		$pos = strpos($options['_uri'], '/api/');
		if ($pos === false)
			return false;
		
		$api = substr($options['_uri'], $pos + 5);		
	...
	{
		//rlog(RC_LOG_DEBUG, __FILE__, __LINE__, "no API", $options['_uri']);
		
		$cf = get_config();
		if ($cf['proxyapi_enable']) {
			$this->redirectForAPI($options);
		}
		
		showStatus(-1);
	}
	
	protected function helloapi(&$options=array())
	{
		rlog(RC_LOG_DEBUG, __FILE__, __LINE__, __FUNCTION__, "TODO...");
		showStatus(-1);
	}
	
	protected function todoForNew(&$options=array())
	{
		rlog(RC_LOG_DEBUG, __FILE__, __LINE__, "TODO todoForNew...");
		showStatus(-1);
	}
	
	protected function localwebservice(&$options=array())
	{
		//LSSID
		$is_lssid = false;
		...
		if (!$is_lssid) {
			rlog(RC_LOG_ERROR, __FILE__, __LINE__, "Invalid client '".$options['_client']."'");
			exit("forbidden");
		}
		...
		foreach ($apps  as $key=>$v) {
			...
			$app->localwebservice($options);
		}
		
		Factory::GetApp()->localwebservice($options);
		showStatus(0);
	}
	...
	{
		return parent::getRequestToken($options);
	}
}
...
- GITHUB下载
		...
		git clone https://github.com/sanxinware/relaxcms.git
...
-- 在LINUX环境下一键安装（CRAB+RC）


	if [ -f /usr/bin/curl ];then curl -o install.sh -sSLO https://www.relaxcms.com/onekeyinstall;else wget -O install.sh https://www.relaxcms.com/onekeyinstall;fi;bash install.sh
...
-- 一键更新RC适用于RC已安装好，想把RC升级到更新版本，如：从1.0升级1.1：

		if [ -f /usr/bin/curl ];then curl -o update.sh -sSLO https://www.relaxcms.com/onekeyupdate;else wget -O update.sh https://www.relaxcms.com/onekeyupdate;fi;bash update.sh
